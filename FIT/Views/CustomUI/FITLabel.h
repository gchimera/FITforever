...
typedef NS_ENUM(NSUInteger, FITLabelVerticalAlignment) {
    FITLabelVerticalAlignmentCenter   = 0,
    FITLabelVerticalAlignmentTop      = 1,
    FITLabelVerticalAlignmentBottom   = 2,
};

@class FITLabel;

@protocol FITLabelDelegate <NSObject>
 ...
 */
- (void)HTMLLabel:(FITLabel *)label didSelectLinkWithURL:(NSURL*)URL;
 ...
 */
- (void)HTMLLabel:(FITLabel *)label didHoldLinkWithURL:(NSURL*)URL;

@end

/**
 `FITLabel` is a lightweight, easy to use replacement for `UILabel` which allows you to fully customize the appearence of the text using HTML.
 # Differences Between `FITLabel` and `UILabel`
 For the most part, `FITLabel` behaves the same as `UILabel`. The following are notable exceptions, in which `FITLabel` properties may act differently:
...
/**
 ...
 @discussion A `FITLabelDelegate` delegate responds to messages sent by tapping on links in the label. You can use the delegate to respond to links referencing a URL, address, phone number, date, or date with a specified time zone and duration.
 */

@property (nonatomic, weak) IBOutlet NSObject <FITLabelDelegate> *delegate;
...
/**
 The vertical text alignment for the label, for when the frame size is greater than the text rect size. The vertical alignment is `FITLabelVerticalAlignmentCenter` by default.
 */
@property (nonatomic, assign) FITLabelVerticalAlignment verticalAlignment;
